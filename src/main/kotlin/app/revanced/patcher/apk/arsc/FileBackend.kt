...
internal open class ArchiveBackend(private val path: String, protected val store: EncodeManager) : FileBackend {
    ...
    override fun load(): ByteArray = source!!.openStream().use { it.readAllBytes() }
...
internal class XmlBackend(path: String, store: EncodeManager) : ArchiveBackend(path, store) {
    ...
    override fun load(): ByteArray = ByteArrayOutputStream().also {
        loadXml().save(it, false)
    }.toByteArray()
...
internal class ManifestBackend(private val store: EncodeManager) : FileBackend {
